...
const HOA_GUIDE_PLACEMENT_ID = 43
...
function mapFaqCategory(title = "", bodyText = "") {
  ...
  if (/\b(rule|rules|covenant|covenants|restriction|restrictions|violation|violations)\b/.test(text)) return "rules"
  return "general"
}
...
const QUESTION_TITLE_OVERRIDE_BY_ID = {
  35665: "What is the HOA violation and appeals process at Pristine Place?",
  26765: "When do I need an ACC permit or change request form?",
  26530: "What are the lot coverage and building restrictions in Pristine Place?",
  26294: "How do I appeal an HOA fine at Pristine Place?",
  26221: "What should I do if I receive an HOA violation notice?",
}
...
async function fetchAllHoaGuideArticles() {
  const all = []
  let page = 1
  let totalPages = 1

  while (page <= totalPages) {
    const url = `${WP_BASE_URL}?per_page=100&page=${page}&article-placement=${HOA_GUIDE_PLACEMENT_ID}`
...
async function main() {
  ...
  const wpItems = await fetchAllHoaGuideArticles()
  const filteredItems = wpItems.filter((wp) => {
    const classes = Array.isArray(wp.class_list) ? wp.class_list : []
    return classes.some((c) => String(c).toLowerCase() === "article-placement-hoa-guide")
  })

  if (!filteredItems.length) {
    console.log("No portal_article records found for article-placement=43 (HOA Guide).")
    return
  }

  console.log(`Fetched ${wpItems.length} records from WP query and kept ${filteredItems.length} HOA Guide records.`)
  ...
  for (const wp of filteredItems) {
    ...
    const question = QUESTION_TITLE_OVERRIDE_BY_ID[wp.id] || titleAsQuestion(title)
  ...
  console.log("HOA Guide FAQ import complete.")
}

main().catch((error) => {
  console.error("HOA Guide FAQ import failed:", error)
  process.exit(1)
})
