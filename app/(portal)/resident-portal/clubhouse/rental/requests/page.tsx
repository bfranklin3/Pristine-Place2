...
import { listClubhouseRentalRequestsForResident } from "@/lib/clubhouse-rental/repository"

export const metadata: Metadata = {
  title: `My Clubhouse Rental Requests | ${siteConfig.name} Resident Portal`,
  description: "Review your submitted clubhouse rental requests and current statuses.",
}
...
export default async function MyClubhouseRentalRequestsPage() {
  await requireApprovedPortalAccess()
  const { userId } = await getPortalSession()
  const requests = userId ? await listClubhouseRentalRequestsForResident(userId) : []
    ...
    <>
      <section className="hero-section" style={{ background: "var(--pp-navy-dark)" }}>
        ...
        <div className="hero-content stack" style={{ gap: "var(--space-s)" }}>
          ...
          <h1 className="hero-title">My Clubhouse Rental Requests</h1>
          <p className="hero-subtitle" style={{ maxWidth: "56ch" }}>
            Review your submitted clubhouse rental requests and see whether any additional action is required.
          </p>
        </div>
      </section>
            ...
            {requests.length === 0 ? (
              <div className="card" style={{ padding: "var(--space-l)", background: "#fffef9" }}>
                <p style={{ margin: 0, color: "var(--pp-slate-700)" }}>
                  You have not submitted any clubhouse rental requests through the new workflow yet.
                </p>
              </div>
            ) : (
                  ...
                  <article key={request.id} className="card" style={{ padding: "var(--space-l)", background: "#fffef9", border: "1px solid #e5efe8" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap" }}>
                      <div style={{ display: "grid", gap: "0.35rem" }}>
                        <h3 style={{ margin: 0, color: "var(--pp-navy-dark)" }}>{request.eventType || "Clubhouse Rental Request"}</h3>
                      ...
                      <div style={{ display: "grid", gap: "0.5rem", justifyItems: "end" }}>
                        ...
                        <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap", justifyContent: "flex-end" }}>
                          ...
                          <Link href={`/resident-portal/clubhouse/rental/requests/${request.id}`} className="btn btn-secondary">
                            View Request
                          </Link>
                        </div>
                      </div>
